...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between rounded-md border border-border/50 bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-brand-500 disabled:cursor-not-allowed disabled:opacity-50",
      className
    )}
    {...props}
  >
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    className={cn(
      ...
      // Default text color: Light mode #111827 (gray-900), Dark mode #F9FAFB (gray-50)
      "text-[#111827] dark:text-[#F9FAFB]",
      // Hover state: Light #EFF6FF, Dark #1E3A8A
      "focus:bg-[#EFF6FF] dark:focus:bg-[#1E3A8A]",
      // Selected option: Blue background (#3B82F6), White text
      "data-[state=checked]:bg-[#3B82F6] data-[state=checked]:text-white dark:data-[state=checked]:text-white",
